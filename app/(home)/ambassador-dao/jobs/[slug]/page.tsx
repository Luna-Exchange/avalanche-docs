      ...
      <div className="flex items-center gap-5">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {job.skills.length > 0 ? (
              job.skills.map((skill: { name: any; }, index: Key | null | undefined) => (
                <div key={index}>
  ...
  return (
    <div className="bg-[#111] p-4 rounded-md border border-gray-800 sticky top-6">
      ...
      <div className="mb-6">
        ...
        {job?.skills?.length > 0 ? <div className="flex flex-wrap gap-2">
          {job?.skills?.map(
      ...
      {isModalOpen && (
        <JobApplicationModal id={job.id} customQuestions={job?.custom_questions} isOpen={isModalOpen} onClose={()=>setIsModalOpen(false)} />
      )}
    </div>
  );
};
...
const AmbasssadorDaoSingleJobPage = () => {
  ...
  const extractDescriptionData = (apiResponse: { description: string; }) => {
    const descriptionParagraphs = apiResponse.description
      ? apiResponse.description.split('\n\n').filter(para => para.trim() !== '')
      : [];
        const titleParagraph = descriptionParagraphs.length > 0 
